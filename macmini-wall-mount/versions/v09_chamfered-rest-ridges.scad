...
module rest_ridge_right() {
    // Mini base sits on the top (z = zmb); underside is a 45-deg chamfer -> prints in place, no overhang
    pts = [
        [rx - ridge_w, zmb],            // inner tip (top)
        [rx + wall_th, zmb],            // outer top (into rail)
        [rx + wall_th, zmb - ridge_w],  // outer bottom
        [rx,           zmb - ridge_w]   // rail inner-face bottom; closes to the tip via the 45-deg chamfer
    ];
    translate([0, sy, 0])
        rotate([90, 0, 0])
...
cut_ports  = true;
port_grow  = 1.0;     // widen each connector port (clearance + easier print-in-place)
...
module port_slot(sgn, yc, w, h, zc = port_z, grow = port_grow) {
    translate([sgn * 65, yc, zc])
        rotate([0, 90, 0]) {
            if (h < 0)
                cylinder(h = 40, d = w + grow, center = true);   // round
            else {
                W = w + grow; H = h + grow; R = min(W, H) / 2;   // obround (fully rounded ends)
...
module port_cuts() {
    ...
    if (led) port_slot(1, led_y, led_d, -led_d, led_z, 0);   // round LED hole (kept small, no widening)
}
