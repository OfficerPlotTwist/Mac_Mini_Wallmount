...
| Ver | File | Key change |
|-----|------|-----------|
| v15 | `v15_airflow-vents_finger-corner-cleanup.scad` | **Current.** **Airflow vents** (`airflow`, `air_*`): 8 holes bored through the backplate in the gaps between the radial struts (skipping the finger-hole corner). **Junk cleanup**: the finger cylinder ran along the −X,−Y corner and shredded that corner's fan into stub fragments → that one corner's fan + shelves are now dropped (`pb_skip_a`, auto-derived from the button angle = 180°). Also: project rescaled to **mm** in Blender and the GLB Mini reference reseated cleanly. |
