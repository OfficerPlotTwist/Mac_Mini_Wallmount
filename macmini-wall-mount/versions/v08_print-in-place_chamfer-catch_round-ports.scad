...
module side_hook_right() {
    // catch with a 45-deg CHAMFERED underside -> prints in place (back on bed) with no overhang
    pts = [
        [rx,            zmt],                          // rail inner top
        [rx - lip_flat, zmt + lip_flat],              // catch tip (45-deg chamfer up from the rail)
        [rx - lip_flat, zmt + lip_flat + wall_th],    // tip top
        [rx + wall_th,  zmt + lip_flat + wall_th],    // outer top
        [rx + wall_th,  zmt]                           // rail outer top
    ];
    translate([0, sy, 0])
        rotate([90, 0, 0])
