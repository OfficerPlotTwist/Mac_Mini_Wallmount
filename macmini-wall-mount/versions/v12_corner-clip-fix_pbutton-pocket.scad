...
pb_clear    = true;       // clear the ribs beneath the power button (finger/tool access to press it)
pb_x        = -47.9;      // power-button centre (harness coords) — located from Macmini.glb 'power button'
pb_y        = -47.9;      //   mesh, with the Mini seated in the harness: back/-X side, down by the -Y corner
pb_r        = 10;         // radius of the round access pocket (stays ~2mm clear of the corner wall)
pb_rib_h    = 3;          // ribs left this tall ABOVE the backplate base beneath the button
...
// Fan of struts from the pad ring out to a wide span of the +X side rail.
module join_fan_rightX() {
    for (i = [0 : join_n-1]) {
...
module join_walls() {
    linear_extrude(join_h) {
        join_fan_rightX(); mirror([1,0,0]) join_fan_rightX();        // to the two side rails
        for (a = [0, 90, 180, 270]) rotate([0, 0, a]) join_fan_corner();  // to the four corners
    }
}
...
module support_shelves() {
    for (sx = [1, -1])
        for (i = [0 : join_n-1]) {
            ...
            rail_shelf(sx, f * join_spread);   // any shelf inside the button pocket is removed by pb_cut()
        }
...
module pb_cut() {
    // round access pocket centred on the power button: clears ribs/shelves above pb_rib_h so a
    // finger/tool can reach up through the standoff gap to press the (wall-facing) button.
    translate([pb_x, pb_y, back_th + pb_rib_h])
        cylinder(h = zmt, r = pb_r);
}
...
web_min    = 1.35;    // min material left between adjacent cutouts = 3 perimeters @0.45mm line.
                      //   The back rail is crowded (the 3 Thunderbolt sockets sit 8.5mm
                      //   apart on the real Mini and can't be moved), so width growth is
                      //   clamped per-port to keep every web >= this. Isolated ports
                      //   (Power, both USB-C, headphone) reach the full +boot_grow width.
...
module port_slot(sgn, yc, w, h, wg, hg, zc = port_z) {
    translate([sgn * 65, yc, zc])
        rotate([0, 90, 0]) {
            if (h < 0)
                cylinder(h = 40, d = w + wg, center = true);     // round (width grow = diameter grow)
            else {
                W = w + wg; H = h + hg; R = min(W, H) / 2;       // obround (fully rounded ends)
                linear_extrude(height = 40, center = true)
                    offset(R) square([max(H - 2*R, 0.01), max(W - 2*R, 0.01)], center = true);
            }
        }
}
module port_cuts() {
...
module mount() {
    difference() {
        ...
        if (pb_clear) pb_cut();                      // power-button clearance (shortens ribs to 3mm there)
