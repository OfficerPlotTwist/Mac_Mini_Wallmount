...
pb_skip_a   = ((round((atan2(pb_y,pb_x)-45)/90)*90) % 360 + 360) % 360;  // corner fan/shelf to DROP so the
                          // finger cylinder doesn't shred it into junk stubs (=180 for the -X-Y corner)
...
module join_walls() {
    linear_extrude(join_h) {
        ...
        for (a = [0, 90, 180, 270]) if (!(pb_clear && a == pb_skip_a))
            rotate([0, 0, a]) join_fan_corner();                     // corners (skip the finger-hole corner)
    }
}
