...
pb_clear    = true;       // cut a THROUGH-hole at the power button so it's reachable from behind
pb_x        = -47.9;      // power-button centre (harness coords) — from Macmini.glb 'power button' mesh
pb_y        = -47.9;      //   (back/-X side, by the -Y corner); Mini seated in harness to confirm
pb_r        = 10;         // access-hole radius (Ø20; clears the pad + corner wall, ~5mm off plate edge)
...
module pb_cut() {
    // The power button faces the wall and is trapped between the Mini and the backplate, so cut a
    // THROUGH-hole from the back face up to the button — clears the backplate AND any struts/shelves
    // in that column, leaving a clear channel to press it from behind. (Pad & corner wall untouched.)
    translate([pb_x, pb_y, -1])
        cylinder(h = zmt + 2, r = pb_r);
}
