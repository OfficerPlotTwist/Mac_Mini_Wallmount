...
pb_clear    = true;       // shorten ribs + drop the shelf beneath the power button
pb_side     = -1;         // X side of the power button (-1 = back/-X rail)  *** VERIFY on real Mini ***
pb_y        = 0;          // Y centre of the power button on that rail        *** SET to the real spot ***
pb_zone     = 34;         // Y width of the clearance zone
pb_clear_h  = 3;          // ribs shortened to this height (mm from base) beneath the button
pb_depth    = 22;         // how far inward from the rail the clear zone reaches
...
module support_shelves() {
    for (sx = [1, -1])
        for (i = [0 : join_n-1]) {
            ...
            yr = f * join_spread;
            pb = pb_clear && (sx == pb_side) && (abs(yr - pb_y) < pb_zone/2);
            if (!pb) rail_shelf(sx, yr);
        }
...
module pb_cut() {
    x0 = pb_side * rx;                                  // rail inner face on the button side
    translate([min(x0, x0 - pb_side*pb_depth), pb_y - pb_zone/2, pb_clear_h])
        cube([pb_depth, pb_zone, zmt]);
}
