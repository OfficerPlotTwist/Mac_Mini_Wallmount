...
module rest_ridge_right() {
    // Mini base sits on the top (z = zr); underside is a 45-deg chamfer -> prints in place, no overhang
    zr = zmb + ridge_lift;              // lifted above the spacer plane to meet the recessed perimeter
    pts = [
        [rx - ridge_w, zr],            // inner tip (top)
        [rx + wall_th, zr],            // outer top (into rail)
        [rx + wall_th, zr - ridge_w],  // outer bottom
        [rx,           zr - ridge_w]   // rail inner-face bottom; closes to the tip via the 45-deg chamfer
    ];
    translate([0, sy, 0])
        rotate([90, 0, 0])
            linear_extrude(height = 2*sy)
                polygon(pts);
}
